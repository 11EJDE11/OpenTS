...
title: Pickup Crate
summary: Satisfied when the object carrying the tag collects a crate whose overlay is marked as a trigger crate.
caveats:
  - Only a crate whose overlay type carries `CrateTrigger=yes` makes the offer. A crate without it is collected in silence, and which of the two a crate is depends on the overlay type rather than on how the crate reached the map.
