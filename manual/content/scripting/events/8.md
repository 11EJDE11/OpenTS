...
id: TEVENT_ANY
title: Any Event
summary: Satisfied by every offer without examining anything, so a trigger carrying it alone fires on the first frame it is enabled.
caveats:
...
The event describes a lasting condition rather than a moment, which exempts it from [the naming requirement](/systems/trigger-springing/#temporal-and-standing-events) that decides most events, and it then carries no further test. Whatever an offer was about, the answer is yes.
...
That matters for a trigger that pairs this event with a real one. A [semi-persistent](/systems/trigger-springing/#tag-lifetimes) tag counts objects and cells only, and joining two engine lists adds nothing to that count, so pairing this event with another does not rescue a semi-persistent tag that rides on nothing.
