...
The placement center is the durable half of what the on branch does. Nothing recalculates it afterward — unlike the averaged base center, which is worked out again from the house's structures every time one is placed, destroyed or captured — so a house sent to build around one construction yard goes on building around that spot for the rest of the scenario, even after that structure is gone. [Choosing a spot](/systems/ai-base-building/#choosing-a-spot) owns what the search does with it.
