...
Every object changes hands through the shared routine that [an engineer walking into a structure](/systems/capture/#what-changes-hands) uses, one after another, and that routine settles the points, the inventory counts, the cleared orders and everything a captured structure carries with it.

Two consequences follow from doing that to a whole side at once. Each object is booked as a kill for the receiving house, so a handover of any size moves that house's kill counts and points by the full value of everything it received. And each object's own tag is offered the [`Entered by...`](/mapping/events/tevent-player-entered/) event as it changes hands, so every entered trigger in the scenario that rides on one of those objects is examined in the same frame.

[Change House](/mapping/actions/taction-change-house/) is the narrower form of the same handover, aimed at the objects carrying the trigger's own tag rather than at a house.
