...
Each pass sells every structure of the house that is on the map and still standing, one after another, and then runs the same sweep the hunt action runs — so the lasting target bias that action leaves on the house is set here as well, and reset on every pass.
