...
:::danger[A task force that builds nothing faults]
The new team is taken hold of before the delivery checks whether anything was created at all. A member entry is accepted with a count of zero — `0=0,E1` is counted as a member of the force and builds no infantryman — so a task force whose entries all read that way is delivered with nothing in hand and faults on the spot. Both reinforcement actions reach it the same way.
:::
