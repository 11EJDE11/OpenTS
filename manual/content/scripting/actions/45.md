...
The gap is not a corner case. A storm ordered by [Ion Storm start...](/mapping/actions/taction-ion-storm-start/) waits [`IonStormWarning`](/keys/ionstormwarning/) seconds before it breaks, and this action can do nothing with it for the whole of that wait. A scenario that means to call a storm off has to reach it after the break rather than during the warning, and only a warning of zero closes the gap entirely.
