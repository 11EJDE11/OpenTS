...
see_also: ["system:crates", DeploysInto]
when_omitted:
  kind: value
  value: none
---
...
- With bases enabled, each house that is not passive is given one at its starting waypoint, and it is the object a house's flag is attached to in capture-the-flag. Enabling bases also takes one off the lobby's unit count to pay for it.
- The random starting units are drawn from the [`AllowedToStartInMultiplayer=yes`](/keys/allowedtostartinmultiplayer/) types with this one held out, and the average price that decides how many are drawn leaves it out too.
