...
The LAN and serial handlers each play it again immediately after the line is added, so a message arriving from another player over those links is heard twice. A Westwood Online page is doubled the same way while a match is running. A message the game raises for itself is heard once, and a line too long for the message area is split, playing the sound once per piece.
:::
