...
Wall stitching is not a flag on a type. The named type is the one the [connection logic](/systems/walls-and-gates/#connection-frames) accepts as a continuation of a brick or sandbag wall, and only from the north and the south, so the run reads through the gate along that axis while a wall arriving from the east or west stops dead at it. `GAWALL` and `GASAND` are the stock overlays this covers.
