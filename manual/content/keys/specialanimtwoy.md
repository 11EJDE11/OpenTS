...
The vertical half of the second special slot's offset. It carries the [`SpecialAnimTwo`](/keys/specialanimtwo/) animation down the screen from the point the structure is drawn at, and a negative figure lifts it above that point instead. The offset pins the animation to a point on the artwork rather than to a cell, so it holds wherever the structure stands.
