...
- A human house forms an alliance with a house that is not multiplayer-passive, an alliance with a computer house included. The check runs while the scenario is being set up only in an internet game, so alliances a map declares in advance do not set it off elsewhere.
