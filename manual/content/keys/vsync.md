...
The wait is the reason it is off by default. A present that blocks holds up the game loop that asked for it, and that shows up as added lag between moving the mouse and seeing the result. The pointer itself is unaffected either way, because the system draws it over the game's picture rather than in it.
