...
summary: How far right of the structure's drawing point the third active slot's animation sits, in screen pixels.
see_also: ["ActiveAnimThree", "ActiveAnimThreeY"]
when_omitted:
  kind: value
  value: "0"
---

The horizontal half of the third slot's offset. It carries the [`ActiveAnimThree`](/keys/activeanimthree/) animation right across the screen from the point the structure is drawn at, and a negative figure moves it left of that point instead. The offset pins the animation to a point on the artwork rather than to a cell, so it holds wherever the structure stands.
