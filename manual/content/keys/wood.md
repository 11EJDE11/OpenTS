...
A TerrainType takes damage from this warhead and from no other. Damage processing on a terrain object begins by testing the flag and reports nothing at all when it is absent, so a warhead left at `no` cannot fell a tree however much damage it carries. A vehicle likewise treats a terrain object in its path as destroyable only when the weapon it would use against it carries the flag, and as impassable otherwise.
