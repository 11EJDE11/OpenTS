...
Neither is reached. The test sits inside the routine that puts a figure on an object as a target, and the only thing that calls that routine is the pass in which a computer house picks a building to aim a superweapon at — a pass nothing invokes. What a computer house does weigh when it chooses a target is traced under [target selection](/systems/target-selection/), and cargo has no part in it.
