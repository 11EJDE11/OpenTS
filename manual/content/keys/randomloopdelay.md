...
:::danger[A paused loop can put a network game out of step]
The pause is drawn from the generator the engine keeps for effects that must not influence the outcome, and that generator is not held in step between machines. A looping animation whose opening side effects reach the game — one carrying [`Damage=`](/keys/damage/#scope-animtype), `TiberiumChainReaction=yes`, [`Scorch=`](/keys/scorch/), [`Crater=`](/keys/crater/#scope-animtype) or [`Flamer=`](/keys/flamer/) — applies those effects on different frames on each machine, which desynchronizes the game.
:::
