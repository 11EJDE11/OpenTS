...
/// <summary>
/// Compresses a block of data with the LZO1X-1 algorithm.
/// This is the low level compressor that the public entry point hands a block to once it
/// is long enough to be worth searching for matches. Literal runs and back references are
/// written to the destination as they are found.
/// </summary>
/// <param name="out_len">Set to the number of compressed bytes written.</param>
/// <param name="wrkmem">Work memory used to hold the compressor's match dictionary.</param>
/// <returns>Returns with an LZO error code; LZO_E_OK if the block was compressed.</returns>
/// <remarks>The work memory must be at least LZO1X_MEM_COMPRESS bytes long, and the
/// destination buffer must be big enough to hold data that fails to compress.</remarks>
...
/// <summary>
/// Compresses a block of data into the LZO1X-1 stream format.
/// This is the public entry point to the compressor. A block too short to be worth
/// matching is copied through as a single literal run, and every stream is finished off
/// with an end of stream marker so that the decompressor knows where to stop.
/// </summary>
/// <param name="out_len">Set to the number of compressed bytes written.</param>
/// <param name="wrkmem">Work memory used to hold the compressor's match dictionary.</param>
/// <returns>Returns with an LZO error code; LZO_E_OK if the block was compressed.</returns>
/// <remarks>The work memory must be at least LZO1X_MEM_COMPRESS bytes long. Be sure that
/// the destination buffer is big enough to hold data that fails to compress.</remarks>
