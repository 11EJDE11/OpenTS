...
void Draw_Shape(Surface & surface, ConvertClass & convert, ShapeSet const * shapefile, int shapenum, Point2D const & point, Rect const & window, ShapeFlags_Type flags, unsigned char const * remap, int height_offset, ZGradientType zgrad, int intensity, ShapeSet const * z_shapefile, int z_shapenum, Point2D z_off)
{
	...
	*/
	...
	if (z_shapefile != NULL) {
		zpoint -= ((Point2D(width, height) / 2) - Point2D(rect.X, rect.Y));
	}
	...
	*/
	if (shapefile->Is_RLE_Compressed(shapenum)) {
		...
		if (blitter != NULL) {
			Bit_Blit(surface, window, Rect(x, y, rect.Width, rect.Height), shape, shape.Get_Rect(), rect, *blitter, height_offset, zgrad, intensity, 0);
		}
	}
 ...
 *   05/27/1997 JLB : Created.                                                                 *
 *=============================================================================================*/
void Blit_Block(Surface & surface, ConvertClass & convert, Surface const & source, Rect const & sourcerect, Point2D const & point, Rect const & window, unsigned char const * remap, Blitter const * blitter, int height_offset, ZGradientType zgrad, int intensity, int unknown)
{
	convert.Set_Remap(remap);
	...
	*/
	Bit_Blit(surface, window, Rect(point.X, point.Y, sourcerect.Width, sourcerect.Height), source, source.Get_Rect(), sourcerect, *blitter, height_offset, zgrad, intensity, unknown);
}
