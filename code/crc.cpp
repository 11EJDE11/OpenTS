...
/// <summary>
/// Submits one boolean datum to the CRC engine.
/// This is the boolean flavor of the byte sized submission operator.
/// </summary>
void CRCEngine::operator() (bool datum)
{
	Submit(datum != false);
}
...
/// <summary>
/// Submits one word sized datum to the CRC engine.
/// </summary>
void CRCEngine::operator() (short datum)
{
	/// Union used to ensure the datum type is passed into the engine as a composite.
	union {
		short Composite;
		short Datum;
	} _datum;
	...
	(*this)(&_datum.Composite, sizeof(_datum.Composite));
}


/// <summary>
/// Submits one long word sized datum to the CRC engine.
/// </summary>
void CRCEngine::operator() (int datum)
{
	/// Union used to ensure the datum type is passed into the engine as a composite.
	union {
		int Composite;
...
/// <summary>
/// Submits one floating point datum to the CRC engine.
/// The value is submitted as its raw bit pattern rather than as a number, so two values
/// that compare equal but differ in representation will not agree on the CRC.
/// </summary>
void CRCEngine::operator() (float datum)
{
	/// Union used to ensure the datum type is passed into the engine as a composite.
	union {
		int Composite;
...
/// <summary>
/// Submits one double precision datum to the CRC engine.
/// The value is submitted as its raw bit pattern rather than as a number.
/// </summary>
void CRCEngine::operator() (double datum)
{
	double _datum = datum;

	(*this)(&_datum, sizeof(_datum));
}


/// <summary>
/// Submits a null terminated string to the CRC engine.
/// The terminator itself is not submitted, so a string contributes the same value here
/// as its characters would if submitted one at a time. A NULL pointer is ignored.
/// </summary>
/// <param name="buffer">Pointer to the string to submit. It will not be modified.</param>
void CRCEngine::operator() (const char * buffer)
{
	if (buffer != NULL) {
		(*this)(buffer, strlen(buffer));
	}
}
...
/// This is the low level routine that the CRC engine and the string flavor both build
/// upon. The block will not be modified.
/// </summary>
...
/// <summary>
/// Calculates the CRC of a null terminated string.
/// Use this routine where a name or a key must be reduced to a single integer that can
/// be compared or hashed cheaply. The terminator itself does not contribute.
/// </summary>
/// <param name="crc">The CRC value to accumulate onto. Pass the result of a previous
/// call in order to chain several strings into one value.</param>
/// <returns>Returns with the CRC of the string.</returns>
