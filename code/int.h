...
class Int {
	public:
		...
		*/
		...
		unsigned MaxBitPrecision(void) const {return(PRECISION*(sizeof(unsigned int)*CHAR_BIT));}
...
//		friend Int<PRECISION> operator % (digit a, const Int<PRECISION> & b) {return(Int<PRECISION>(a) % b);}
		...
		*/
		bool operator == (const Int &b) const {return(memcmp(&reg[0], &b.reg[0], (MAX_BIT_PRECISION/CHAR_BIT))==0);}
		bool operator != (const Int& b) const {return(!(*this == b));}
		bool operator > (const Int & number) const {return(XMP_Compare(&reg[0], number, PRECISION) > 0);}
		bool operator >= (const Int & number) const {return(XMP_Compare(&reg[0], number, PRECISION) >= 0);}
		bool operator < (const Int & number) const {return(XMP_Compare(&reg[0], number, PRECISION) < 0);}
		bool operator <= (const Int & number) const {return(XMP_Compare(&reg[0], number, PRECISION) <= 0);}
		...
		*/
		...
		Int times_b_mod_c(Int const & multiplier, Int const & modulus) const {
			Int result;
			Error=XMP_Prepare_Modulus(modulus, PRECISION);
...
//		friend Int<PRECISION> a_exp_b_mod_pq(const Int<PRECISION> & a, const Int<PRECISION> & ep, const Int<PRECISION> & eq, const Int<PRECISION> & p, const Int<PRECISION> & q, const Int<PRECISION> & u);
	...
	private:
		...
		struct RemainderTable
		{
			...
			bool HasZero(void) const {return(HasZeroEntry);}
