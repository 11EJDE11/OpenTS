...
const char *GadgetClass::CurrentColorScheme = DEFAULT_GADGET_SCHEME;


/// <summary>
/// Fetches the color scheme index that the gadgets draw with.
/// This routine resolves the current color scheme name into the index that the drawing
/// code works with. The scheme is a class-wide setting, so every gadget shares it.
/// </summary>
/// <returns>Returns with the ID of the current color scheme, or zero if the name is not
/// recognized.</returns>
int GadgetClass::Get_Color_Scheme(void)
{
	ColorScheme * scheme = Fetch_Scheme_By_Name(CurrentColorScheme);
 ...
 *=============================================================================================*/
GadgetClass::GadgetClass(int x, int y, int w, int h, unsigned flags, bool sticky) :
 ...
 *   01/15/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
LinkClass * GadgetClass::Remove(void)
{
	Clear_Focus();
 ...
 *   01/15/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
LinkClass * GadgetClass::Get_Next(void) const
{
	return(BASECLASS::Get_Next());
}
 ...
 *   01/15/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
LinkClass * GadgetClass::Get_Prev(void) const
{
	return(BASECLASS::Get_Prev());
}
...
void GadgetClass::Draw_All(bool forced)
{
	...
	while (gadget != NULL) {
		...
		gadget = (GadgetClass *)gadget->Get_Next();
	}
}
			...
			*/
			...
			while (next_button != NULL) {
				...
				*/
				...
				next_button = (GadgetClass *)next_button->Get_Next();
			}
		}
	}
	return(key);
...
{
	if (IsSticky && (flags & (LEFTPRESS|RIGHTPRESS))) {
		StuckOn = this;
	}
	if (StuckOn == this && (flags & (LEFTRELEASE|RIGHTRELEASE))) {
		StuckOn = 0;
	}
}
...
/// <summary>
/// Sets the dimensions of this gadget.
/// This routine is the companion to Set_Position. Use it to resize the screen region
/// that the gadget considers "its own" when a dialog lays out its controls at run time.
/// </summary>
/// <param name="w">The width of the gadget's region, in pixels.</param>
/// <param name="h">The height of the gadget's region, in pixels.</param>
