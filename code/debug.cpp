...
DMonoType MonoPage = DMONO_STRESS;

void ___DebugFunc(void)
{
...
//			case KN_P:
//				{
//					for (int spc = 0; spc < PlayerPtr->SuperWeapon.Count(); spc++) {
...
//				break;
			...
			case (int)KN_Z|(int)KN_ALT_BIT:
				if (map_x == -1) {
					map_x = Map.MapRect.X;
					map_y = Map.MapRect.Y;
					map_width = Map.MapRect.Width;
					map_height = Map.MapRect.Height;
					Map.MapRect.X = 1;
					Map.MapRect.Y = 1;
					Map.MapRect.Width = MAP_CELL_W-2;
					Map.MapRect.Height = MAP_CELL_H-2;
				} else {
					Map.MapRect.X = map_x;
					Map.MapRect.Y = map_y;
					Map.MapRect.Width = map_width;
					Map.MapRect.Height = map_height;
					map_x = -1;
					map_y = -1;
					map_width = -1;
					map_height = -1;
...
//				break;
			...
			case (int)KN_DELETE|(int)KN_SHIFT_BIT:
				if (CurrentObject.Count()) {
					Map.Recalc();
					int damage = 50;
					WarheadTypeClass *warheadtype = WarheadTypeClass::From_Name("SA");
					CurrentObject[0]->Take_Damage(damage, 0, warheadtype);
				}
				break;
			...
			case KN_INSERT:
				if (CurrentObject.Count()) {
					...
					if (Map.PendingObject) {
						...
						Map.PendingObjectPtr = Map.PendingObject->Create_One_Of(Houses[Map.PendingHouse]);
			...
			*/
			case KN_F7:
				if (CurrentObject.Count() && CurrentObject[0]->Is_Techno()) {
					...
					int weapon = 0;
					if (ttype->Weapons[0].Weapon != NULL) weapon = ttype->Weapons[0].Weapon->Range;
//					Set_Logic_Page(SeenBuff);
					Coord center = CurrentObject[0]->Center_Coord();
					Coord center2 = CurrentObject[0]->Fire_Coord(0);

					int white_color = ((DSurface *)LogicalSurface)->Build_Hicolor_Pixel(0xFF,0xFF,0xFF);
					int red_color = ((DSurface *)LogicalSurface)->Build_Hicolor_Pixel(0xFF,0,0);

					for (int r = 0; r < DIR_MAX; r += 10) {
						Point2D point1;
						Point2D point2;
						Dir256 r1 = (Dir256)r;
						Dir256 r2 = (Dir256)((r+10) & DIR_MAX);

						if (TacticalMap->Coord_To_Pixel(Move_Coord(center, r1, sight), point1)) {
							TacticalMap->Coord_To_Pixel(Move_Coord(center, r2, sight), point2);
							LogicalSurface->Draw_Line(Point2D(point1.X, point1.Y+8), Point2D(point2.X, point2.Y+8), white_color);
						}
						if (TacticalMap->Coord_To_Pixel(Move_Coord(center2, r1, weapon), point1)) {
							TacticalMap->Coord_To_Pixel(Move_Coord(center2, r2, weapon), point2);
							LogicalSurface->Draw_Line(Point2D(point1.X, point1.Y+8), Point2D(point2.X, point2.Y+8), red_color);
						}
					}
				}
				break;
