...
CheckListClass::~CheckListClass(void)
{
	while (CheckListClass::Count()) {
		CheckObject * obj = (CheckObject *)BASECLASS::Get_Item(0);

		BASECLASS::Remove_Item(0);
		delete obj;
	}
}
...
int CheckListClass::Add_Item(char const * text)
{
	...
	return(BASECLASS::Add_Item((char const *)obj));
}


/// <summary>
/// Fetches a pointer to the text of the current selection.
/// This routine will find the text associated with the currently selected entry and return
/// a pointer to it. Use this routine rather than the base list class version, which hands
/// back the check object instead of the text.
/// </summary>
/// <returns>Returns with the text pointer of the selected entry. If there is no selection,
/// then NULL is returned.</returns>
char const * CheckListClass::Current_Item(void) const
{
	CheckObject * obj = (CheckObject *)BASECLASS::Current_Item();
	if (obj) {
		return(obj->Text);
	}
	return(0);
...
void CheckListClass::Draw_Entry(int index, int x, int y, int width, int selected)
{
	...
	if (obj) {
		...
		if (selected) {
			...
			LogicalSurface->Fill_Rect (Rect(x, y, width, LineHeight), NormalDrawer->Convert_Pixel(ColorSchemes[scheme]->Shadow));
		} else {
		...
		Conquer_Clip_Text_Print(buffer, *LogicalSurface, LogicalSurface->Get_Rect(), Point2D(x, y), ColorSchemes[scheme], TBLACK, flags, width, Tabs);
	}
}
