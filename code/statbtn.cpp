	...
	PrintFlags(style)
{
	...
	*/
	...
	if (w == -1 || h == -1) {
		...
		if (w == -1) {
			Width = font->String_Pixel_Width(String);
		}
		if (h == -1) {
...
/// <summary>
/// Destroys the static button.
/// The button holds its own copy of the text it displays, so that copy is freed here.
/// </summary>
StaticButtonClass::~StaticButtonClass(void)
{
	if (String != NULL) {
...
void StaticButtonClass::Draw_Background(void)
{
	...
	*/
	if (Background.Get_Buffer() != NULL) {
		From_Buffer(*LogicalSurface, Rect(X, Y, Width, Height), Background);
	}
}
...
void StaticButtonClass::Draw_Text(char const * text)
{
	...
	*/
	if (String != NULL) {
		...
		Fancy_Text_Print(text, *LogicalSurface, LogicalSurface->Get_Rect(), Point2D(x, Y), ColorSchemes[BASECLASS::Get_Color_Scheme()], TBLACK, PrintFlags);
	}
}
